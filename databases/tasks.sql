...
SELECT s.name, COUNT(t.id) FROM status s LEFT JOIN task t ON t.status_id = s.id GROUP BY s.name;

-- 10. Get the names of all statuses, sorted by the status with most tasks first.
SELECT s.name, COUNT(t.id) FROM status s LEFT JOIN task t ON t.status_id = s.id GROUP BY s.name ORDER BY COUNT(t.id) DESC;
